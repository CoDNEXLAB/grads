...
if (($FHour == 000) || ($FHour == 006)) then
	set dataFile = `find ${dataDir}/${modDir}/*${ModInit}00F${FHour}.* ! -name '*.gefs_' ! -name '*.idx'| tail -n1`
	#set ctlFile = `echo ${dataFile} | sed -e "s/00F[^ ]...gefs_.../00F%f3.gefs/"`
	set ctlFile = `echo ${dataFile} | sed -e "s/00F[^ ]../00F%f3.gefs/"`
