...
*
* Usage:   meteogram_eta <yyyymmddhh> <lon> <lat> <e> <name> <stid>
* Example: meteogram_eta  2003022100   -71    42   e  Boston  bos
*
...
* Open the data file
'reinit'
_dummydset = 'dset ^'stid'.eta.dummy.dat'
_dummydata = stid%'.eta.dummy.dat'
_dummyfile = stid%'.eta.dummy.ctl'
_baseurl = 'http://cola8.igex.org:9191/dods/eta/'
_dataset = 'eta'date
opened = 0
while (!opened)
  say 'sdfopen '_baseurl%_dataset
  'sdfopen '_baseurl%_dataset
  say 'rc='rc
  if (!rc)
   opened = 1 
  else 
    say 'File not found. quitting ..'
    'quit'
  endif
endwhile
say 'Generating a meteogram for 'name
...
if (printim)
  say 'Printing Image: 'stid'eta.png'
  'printim 'stid'eta.png x620 y800'
endif

* Remove the dummy files and the process id file
'!rm -f '_dummyfile
'!rm -f '_dummydata
'!rm -f 'stid'.eta.pid'
'quit'
...
'set gxout fwrite'
'set fwrite '_dummydata
'd 'dodsvar
'disable fwrite'

* Write a descriptor file 
rc = write(_dummyfile,_dummydset)
rc = write(_dummyfile,_undef,append)
rc = write(_dummyfile,'xdef 1 linear 1 1',append)
rc = write(_dummyfile,'ydef 1 linear 1 1',append)
rc = write(_dummyfile,_zdef,append)
rc = write(_dummyfile,_tdef,append)
rc = write(_dummyfile,'vars 1',append)
rc = write(_dummyfile,'dummy '_newzsize' -999 dummy',append)
rc = write(_dummyfile,'endvars',append)
rc = close (_dummyfile)

* Open the dummy file, define variable, close dummy file
'open '_dummyfile
line = sublin(result,2)
dfilenum = subwrd(line,8)
'set dfile 'dfilenum
'set lon 1'
'set lat 1'
'set lev '_zbot' '_ztop
'set time '_time1' '_time2
'define 'myvar' = dummy.'dfilenum
'close 'dfilenum
'set dfile 1'
return
...
rc = write(_dummyfile,'xdef 1 linear 1 1',append)
rc = write(_dummyfile,'ydef 1 linear 1 1',append)
rc = write(_dummyfile,_rhzdef,append)
rc = write(_dummyfile,_tdef,append)
rc = write(_dummyfile,'vars 3',append)
rc = write(_dummyfile,'dummy '_newrhzsize' -999 dummy',append)
rc = write(_dummyfile,'dummyu '_newrhzsize' -999 dummy',append)
rc = write(_dummyfile,'dummyv '_newrhzsize' -999 dummy',append)
rc = write(_dummyfile,'endvars',append)
rc = close (_dummyfile)
...
'define 'myvar' = dummy.'dfilenum
if (_units = 'e')
  'define urh = dummyu.'dfilenum'*2.2374'
  'define vrh = dummyv.'dfilenum'*2.2374'
else
  'define urh = dummyu.'dfilenum
  'define vrh = dummyv.'dfilenum
endif
'close 'dfilenum
'set dfile 1'
...
rc = write(_dummyfile,'xdef 1 linear 1 1',append)
rc = write(_dummyfile,'ydef 1 linear 1 1',append)
rc = write(_dummyfile,'zdef 1 linear 1 1',append)
rc = write(_dummyfile,_tdef,append)
rc = write(_dummyfile,'vars 1',append)
rc = write(_dummyfile,'dummy 0 -999 dummy',append)
rc = write(_dummyfile,'endvars',append)
rc = close(_dummyfile)
...
rc = write(_dummyfile,'zdef 1 linear 1 1',append)
rc = write(_dummyfile,_tdef,append)
rc = write(_dummyfile,'vars 2',append)
rc = write(_dummyfile,'dummy1 0 -999 dummy',append)
rc = write(_dummyfile,'dummy2 0 -999 dummy',append)
rc = write(_dummyfile,'endvars',append)
rc = close(_dummyfile)
...
'set z 1'
'set time '_time1' '_time2
'define 'total' = dummy1.'dfilenum
'define 'conv'  = dummy2.'dfilenum
'close 'dfilenum
'set dfile 1'
'set gxout contour'
